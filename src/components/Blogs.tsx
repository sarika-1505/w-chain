    ...
    <div className="w-full bg-[#010514] flex flex-col items-center font-Montserrat pt-0 pb-44 -mb-[80px] lg:-mb-[100px] relative z-10">
      ...
      <section className="w-full relative h-[400px] flex flex-col items-center justify-center bg-[#010514] rounded-b-[40px] md:rounded-b-[60px] overflow-hidden shadow-[0_10px_50px_rgba(13,127,242,0.1)]">
        ...
        <div className="relative z-10 flex flex-col items-center text-center px-4 max-w-[900px] mt-10">
          ...
          <p className="text-white text-[18px] md:text-[20px] leading-relaxed max-w-[800px] font-medium">
            Stay ahead of the curve with cutting-edge perspectives on: Hybrid
            blockchain breakthroughs, Web3's evolving landscape, and W Chain
      ...
      {/* Blogs Grid */}
      <section className="w-full max-w-360 px-4 md:px-10 flex justify-center z-10 relative mt-10">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 md:gap-[30px]">
          {blogsData.map((blog) => (
            <div
              key={blog.id}
              className="w-full max-w-97.5 mx-auto h-162.5 rounded-[24px] border border-white/10 bg-[linear-gradient(180deg,rgba(43,69,115,0.2)_0%,rgba(11,27,61,0.5)_100%)] backdrop-blur-md p-[15px] flex flex-col hover:border-[#4EA5FF]/50 transition-colors shadow-[0_0_30px_rgba(78,165,255,0.02)]"
            >
              {/* Card Image */}
              <div className="w-full h-60.75 relative rounded-2xl overflow-hidden mb-6 bg-[#020B2D] flex-shrink-0">
